  ...
  return (
    <div
      ...
      className={`relative flex gap-3 px-4 py-4 cursor-pointer transition-colors active:bg-paragraph/5
        ${!notification.read ? "bg-brand/8" : "bg-white"}`}
    >
      {/* icon */}
      <div
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="text-[11px] text-paragraph/80 shrink-0 mt-0.5">
            {timeAgo(notification.createdAt)}
          </span>
        </div>
        <p className="text-[14px] text-paragraph/80 mt-1 leading-snug line-clamp-2">
          {notification.body}
        </p>
        {isActionable && (
          <div className="flex items-center self-end w-full gap-1 mt-3">
      ...
      >
        <X size={15} className="text-paragraph/60" />
      </button>
    </div>
  );
}
...
// ── Main page ─────────────────────────────────────────────────
export default function NotificationHomePage() {
  ...
  return (
    <div className="min-h-screen bg-bgBase pb-24">
      <div className="fixed top-0 left-0 right-0 z-20 bg-bgBase backdrop-blur-sm border-b border-paragraph/8 px-4 pt-4 pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {unreadCount > 0 && (
                <span className="bg-brand text-white text-[11px] font-bold px-2 py-0.5 rounded-full min-w-[22] text-center">
                  {unreadCount}
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-1">
            ...
            >
              <SlidersHorizontal size={20} className="text-paragraph/60" />
            </button>
          </div>
        </div>
      </div>
          ...
          {grouped.map((group, gIdx) => (
            <div key={group.label}>
              <div className="px-4 py-5 flex items-center gap-3">
                <span className="text-[13px] uppercase tracking-widest font-semibold text-paragraph/40">
                  {group.label}
                </span>
                <div className="flex-1 h-px bg-paragraph/5" />
              </div>
          ...
          {notifications.length > 0 && (
            <div className="flex justify-center pt-10 pb-2">
              <button
                ...
                className="flex items-center gap-1.5 text-[15px] text-paragraph/40 hover:text-paragraph/60 transition-colors"
              >
                <RefreshCcw size={14} />
