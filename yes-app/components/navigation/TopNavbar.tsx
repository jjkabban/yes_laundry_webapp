...
export default function TopNavbar({ className }: Props) {
  ...
  return (
    <div className="md:h-auto">
      <header
        className={`fixed top-0 left-0 right-0 z-20 h-[64] flex items-center justify-between px-5 xl:px-10 ${className ?? ""}`}
      >
        <div className="absolute inset-0 -z-10 bg-white/80 backdrop-blur-md border-b border-black/6" />
        ...
        {(isMobile || isTablet) && (
          <motion.button
            ...
            className="p-2 pr-1 rounded-lg hover:bg-black/5 transition-colors"
      ...
      {(isMobile || isTablet) && (
        <AnimatePresence>
          {openMenu && (
            <motion.div
              className="fixed inset-0 z-50 0 h-screen bg-black/50"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
